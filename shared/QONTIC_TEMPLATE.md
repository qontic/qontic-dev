...
Pass `movableContainer` to `mountValueRange` to move a whole panel (for example, palette canvases plus slider and labels) within its positioned parent. Without it, the range host itself moves. A separate drag grip keeps movement distinct from range adjustment; arrow keys move the grip and Home restores the initial position. Optional `storageKey` remembers placement. Resizing keeps the panel in bounds.
