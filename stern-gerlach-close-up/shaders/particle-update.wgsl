...
fn fetchSpinorVoxel(p: vec3<i32>) -> Spinor {
  let s = simResI();
  if (p.y < 0 || p.z < 0 || p.y >= s.y || p.z >= s.z) {
    return spinorZero();
  }
  let q = vec3<i32>(wrapIndexX(p.x), p.y, p.z);
  return waveSpinor(wave[voxelIndexI(q)]);
}
...
fn wrapCoordRange(x: f32, lo: f32, hi: f32) -> f32 {
  if (hi <= lo) { return 0.5 * (lo + hi); }
  let width = hi - lo;
  return lo + modf1(x - lo, width);
}
fn periodicStepDelta(fromX: f32, toX: f32, period: f32) -> f32 {
  var dx = toX - fromX;
  if (dx > 0.5 * period) { dx = dx - period; }
  if (dx < -0.5 * period) { dx = dx + period; }
  return dx;
}
fn reflectIntoBox(xPx: vec3<f32>) -> vec3<f32> {
  ...
  return vec3<f32>(
    wrapCoordRange(xPx.x, 0.0, maxX.x),
    reflectCoord(xPx.y, lo.y, hi.y),
    reflectCoord(xPx.z, lo.z, hi.z)
  );
}
fn particleIsFrozen(state: vec4<f32>) -> bool {
  return uni.detector.y > 0.5 && state.w >= uni.detector.x - 1e-4;
}
fn detectorFlashBrightness(currentBrightness: f32) -> f32 {
  let targetBrightness = 2.0;
  let flashDecayTime = 0.2;
  let decay = exp(-uni.physics.w / flashDecayTime);
  return max(targetBrightness, targetBrightness + (currentBrightness - targetBrightness) * decay);
}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  if (particleIsFrozen(state)) {
    particlesOut[id] = vec4<f32>(detectorFlashBrightness(state.x), state.y, state.z, state.w);
    return;
  }
  ...
  let visualX = state.w + periodicStepDelta(x.x, xn.x, xPeriodF());
  if (uni.detector.y > 0.5 && state.w < uni.detector.x && visualX >= uni.detector.x) {
    let t = clamp((uni.detector.x - state.w) / max(1e-6, visualX - state.w), 0.0, 1.0);
    let hit = mix(x, xn, t);
    let flashPeakBrightness = 10000.0;
    particlesOut[id] = vec4<f32>(flashPeakBrightness, hit.y, hit.z, uni.detector.x);
    return;
  }
  particlesOut[id] = vec4<f32>(xn.x, xn.y, xn.z, visualX);
}
