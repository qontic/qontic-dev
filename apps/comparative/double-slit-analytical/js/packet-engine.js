import {physicsHTML,viewsHTML,physicsEquations} from './physics-content.js?v=80';
...
import {histogramLayout} from './packet-model.js?v=80';
import {aperture,sourceCoefficients,sourceTransverse,sourceEnvelope,sampleSource,sampleTransmittedSource,stepSource,sourceProfile} from './source-packet-model.js?v=80';
import {detectorLaw,quantumSchedule,recordWithHit,samplePixel} from './packet-outcomes.js?v=80';
export function mountPacketEngine({core,advanced}){
 ...
 const validity=document.createElement('div');validity.className='packet-validity';validity.setAttribute('role','status');validity.setAttribute('aria-live','polite');panel.append(validity);
 function updateValidity(){
  const lambda=Math.max(1e-12,Number(wavelength)),kLocal=2*Math.PI/lambda;
  const apertureMetric=kLocal*Number(width.value),packetMetric=kLocal*Number(length.value);
  const relativeSpread=1/(2*Math.max(packetMetric,1e-12)),issues=[];
  if(apertureMetric<3.75)issues.push('slit width is below the recommended paraxial range (kσₐ ≈ 4)');
  if(packetMetric<5)issues.push('packet length is below the recommended narrow-band range (kσₓ ≥ 5)');
  if(relativeSpread>=1)issues.push('the corresponding full Gaussian momentum spectrum contains substantial backward momentum');
  validity.classList.toggle('warning',issues.length>0);
  const heading=document.createElement('strong');heading.textContent=issues.length?'Approximation caution':'Approximation check';
  const details=document.createTextNode(' kσₐ='+apertureMetric.toFixed(2)+', kσₓ='+packetMetric.toFixed(2)+', Δk/k≈'+relativeSpread.toFixed(2)+'.'+(issues.length?' '+issues.join('; ')+'.':' Parameters are within the recommended working range.'));
  validity.replaceChildren(heading,details);
 }
 ...
 const separationControls=mountSlitSeparation({
  ...
  onPreview(value){slitPreviewSeparation=value;draw();if(value!==null){const preview={...p,centers:displayCenters()},predicted=sourceProfile(preview,-yOffset,screenHeight/100-yOffset,2049);histogram({profile:predicted});drawDetection();}},
