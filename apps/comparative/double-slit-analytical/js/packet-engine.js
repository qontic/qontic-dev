...
export function mountPacketEngine({core,advanced}){
 ...
 function slitWidthHelp(){const fwhm=2*Math.sqrt(2*Math.log(2))*Number(width.value);width.title='Gaussian transmission σ = '+width.value+' nm. Cyan wall ticks mark the full width at half maximum: '+fwhm.toFixed(1)+' nm (2.355 σ). Overlapping apertures form a combined soft opening.';width.parentElement.querySelector('label').title=width.title;width.parentElement.querySelector('input[type=number]').title=width.title;}slitWidthHelp();
 ...
 const intervalRow=document.createElement('div');intervalRow.innerHTML='<div class="input-group packet-inline"><label for="packet-interval">Packet interval</label><input id="packet-interval" aria-label="Packet interval" title="Time between source launches, in picoseconds of simulation time. Independent of packet speed and width." type="range" min="10" max="300" step="5" value="40"><input aria-label="Packet interval value" type="number" min="10" max="300" step="5" value="40"><output>ps</output></div>';panel.append(intervalRow);const interval=intervalRow.querySelector('input[type=range]');
 function syncPacketInput(control){const number=control.parentElement.querySelector('input[type=number]');number.value=control.value;number.max=control.max;number.min=control.min;control.parentElement.querySelector('output').textContent=control===interval?(particleType==='neutron'?'ns':'ps'):'nm';if(control===interval)interval.title='Time between source launches in '+(particleType==='neutron'?'nanoseconds':'picoseconds')+' of simulation time; independent of packet speed and width.';}
 ...
 function emissionPeriod(){return (+interval.value)*(particleType==='neutron'?1:.001)/timeUnit;}
 function prepare(){
  pulse++;const cohort={born:t,count:p.particles,active:p.particles,pending:0,absorbed:0};pulses.push(cohort);
  for(let i=0;i<p.particles;i++){const a=sampleSource(p);a.cohort=cohort;a.schedule=interpretation==='bohmian'?quantumSchedule(a,p,law):{at:(p.screen-a.x0)/p.k,type:'hit',index:samplePixel(law.weights)};particles.push(a);}
  nParticles+=p.particles;nextEmission=t+emissionPeriod();
 }
 function resetEngine(){
  window.qonticMWBranches?.cancel();pending=[];particles=[];pulses=[];nextEmission=0;visualPhase=0;flash=null;hold=0;lastMode=interpretation;p=config();fingerprint=JSON.stringify([p,screenHeight,particleType]);yOffset=screenHeight/200;
 ...
 function processPending(){
  ...
  addHit(index);if(interpretation==='copenhagen'){flash={index,remaining:.3};}
 }
 function integrate(dt){
  ...
  for(const a of particles){
   ...
   }else if(a.schedule.at<=end-a.cohort.born){
 ...
 function frame(){
  ...
  const branch=window.qonticMWBranches,slow=branch?.enabled,busy=branch?.busy,waiting=slow&&!branch.ready();
  ...
  if(busy||(slow&&pending.length)){
   ...
   while(remaining>1e-10&&isAnimating&&!(slow&&pending.length)&&limit++<4000){
    ...
    const dt=Math.min(step,remaining,nextEmission-t);integrate(dt);remaining-=dt;
 ...
 function drawSourceWave(){
  ...
  const env=sourceGrid.xs.map(x=>{
   ...
   for(const c of pulses){
    const e=sourceEnvelope(x,t-c.born,p),fraction=(c.active+c.pending)/Math.max(1,c.count-c.absorbed),q=e.rho*fraction;
 ...
 function draw(){if(!enabled)return;ensure();syncMode();setupCtx.clearRect(0,0,canvas.width,canvas.height);
  setupCtx.save();if(shown('plot_screen')){setupCtx.globalAlpha=elementOpacity('plot_screen');setupCtx.strokeStyle=colorScreen;if(p.source){
   ...
   setupCtx.strokeStyle='#d4e6ef';setupCtx.lineWidth=6;for(let j=0;j<canvas.height;j+=2){const mask=aperture((j+1)/toCanvasY/100-yOffset,p);setupCtx.globalAlpha=elementOpacity('plot_screen')*(1-mask*mask);setupCtx.beginPath();setupCtx.moveTo(wallX,j);setupCtx.lineTo(wallX,j+2);setupCtx.stroke();}
   // Mark each Gaussian aperture's intensity FWHM; these are scale marks,
   // not hard edges of the soft transmission profile.
   setupCtx.globalAlpha=elementOpacity('plot_screen');setupCtx.strokeStyle='#81e7f5';setupCtx.lineWidth=1.5;
   const half=Math.sqrt(2*Math.log(2))*p.sy*100*toCanvasY;
   for(const center of p.centers){const y=Y(center);setupCtx.beginPath();setupCtx.moveTo(wallX-9,y-half);setupCtx.lineTo(wallX+9,y-half);setupCtx.moveTo(wallX-9,y+half);setupCtx.lineTo(wallX+9,y+half);setupCtx.stroke();}
