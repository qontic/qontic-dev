...
export function mountPacketEngine({core,advanced}){
 ...
 function integrate(dt){
  ...
  for(const a of particles){
   ...
   if(interpretation==='bohmian'){
    ...
    if(event==='hit'){const index=Math.floor((a.y+yOffset)/(screenHeight/100)*p.bins);if(index>=0&&index<p.bins){pending.push({index,cohort:a.cohort});a.cohort.pending++;}else missed++;}
 ...
 function frame(){
  ...
  particles=particles.filter(a=>!a.done||(interpretation==='bohmian'&&realElapsed-(a.finishedAt??realElapsed)<.8));
 ...
 function draw(){if(!enabled)return;ensure();syncMode();setupCtx.clearRect(0,0,canvas.width,canvas.height);
  setupCtx.save();if(shown('plot_screen')){setupCtx.globalAlpha=elementOpacity('plot_screen');setupCtx.strokeStyle=colorScreen;if(p.source){
   ...
   const openings=p.centers.map(center=>[Math.max(0,Y(center)-half),Math.min(canvas.height,Y(center)+half)]).filter(([a,b])=>b>a).sort((a,b)=>a[0]-b[0]);
  ...
  drawWave();drawParticles();histogram();drawDetection();window.qonticScaleOverlay?.update();geometryControls?.update();slitControls?.update();stats();
 }
 ...
 const slitControls=mountSlitWidth({
  host:document.getElementById('canvas-container'),
  getState(){const host=document.getElementById('canvas-container');return {width:+width.value,centers:p?p.centers.map(center=>Y(center)/canvas.height):[],wallFraction:wallX/canvas.width,scaleY:toCanvasY*host.clientHeight/canvas.height,busy:window.qonticMWBranches?.busy,visible:shown('plot_screen')};},
  onPreview(value){slitPreviewWidth=value;draw();if(value!==null){const preview={...p,sy:value/100},predicted=sourceProfile(preview,-yOffset,screenHeight/100-yOffset,2049);histogram({profile:predicted});drawDetection();}},
