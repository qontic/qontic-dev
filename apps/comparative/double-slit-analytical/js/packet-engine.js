...
export function mountPacketEngine({core,advanced}){
 ...
 function integrate(dt){
  ...
  for(const a of particles){
   ...
   if(interpretation==='bohmian'){
    ...
    if(a.done){a.finishedAt=realElapsed;a.cohort.active--;}if(shown('plot_trajectories')){a.path.push([a.x,a.y]);trimTail(a.path,+tailLength.value/100);}
 ...
 function drawSourceWave(){
  ...
  // Phase display uses the strongest local pulse's phase where they overlap.
  const env=sourceGrid.xs.map(x=>{
   let density=0,strongest=0,phase=0;
   for(const c of pulses){
    ...
    density+=q;if(q>strongest){strongest=q;phase=e.phase;}
   }return {rho:density,cos:Math.cos(phase-visualPhase),sin:Math.sin(phase-visualPhase),envelope:Math.min(1,Math.sqrt(density*Math.sqrt(2*Math.PI)*p.sx))};
  });
