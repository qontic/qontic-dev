import {mountPacketGeometry,trimTail,tailOpacity} from './packet-interaction.js?v=47';
...
export function mountPacketEngine({core,advanced}){
 ...
 function prepare(){
  t=0;visualPhase=0;pulseAbsorbed=0;finished=false;pulse++;particles=Array.from({length:p.particles},()=>{const a=sampleSource(p);a.schedule=quantumSchedule(a,p,law);return a;});nParticles+=p.particles;
 }
 ...
 function syncMode(){
  ...
  if(wasPW!==isPW)for(let i=0;i<particles.length;i++){
   ...
   if(!isPW){a.schedule=quantumSchedule(a,p,law,t);a.path=[];}
   else{
 ...
 const geometryControls=mountPacketGeometry({
  ...
  getGeometry(){if(!p)return null;const host=document.getElementById('canvas-container');return {distance:detectorDistance,height:screenHeight,detectorFraction:detectorX/canvas.width,scaleX:toCanvasX*host.clientWidth/canvas.width,scaleY:toCanvasY*host.clientHeight/canvas.height,busy:window.qonticMWBranches?.busy};},
  onCommit(next){if(next.distance===detectorDistance&&next.height===screenHeight)return;document.getElementById('detector-distance-group').setValueInFirstUnit(next.distance);document.getElementById('screen-height-group').setValueInFirstUnit(next.height);setupGeo(false);reset=0;resetEngine();draw();},
 ...
 queueMicrotask(()=>{
  ...
  const origin=document.getElementById('sourceOption');origin.parentElement.classList.remove('bohmian-only');origin.parentElement.style.display='';origin.value='isotropic';origin.querySelector('option[value=isotropic]').textContent='Gaussian source';
  ...
  document.getElementById('source-position-group').setValueInFirstUnit(225);
  whichPathDetector='none';updateWhichPathButton();setupGeo(false);reset=0;resetEngine();draw();
 });
 return {get viewportWidth(){viewWidth=Math.max(viewWidth,(225+detectorDistance)/.7);return viewWidth;},get enabled(){return true;},reset:resetEngine,draw,drawWave,drawParticles,histogram,frame,updateMath,hash,pause(){last=null;if(packetAnimationId!==null)cancelAnimationFrame(packetAnimationId);packetAnimationId=null;}};
}
