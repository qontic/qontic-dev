...
export function mountPacketEngine({core,advanced}){
 ...
 // brightness pumping as the packet arrives or is absorbed.
  ...
  for(let j=0;j<240;j++)for(let i=0;i<320;i++){
   ...
   const gain=1+3*(1-Math.exp(-Math.max(0,sourceGrid.xs[i]-p.wall)/.35)),displayDensity=Math.min(1,rho*gain/Math.max(1e-30,peak));
   const weight=Math.min(1,2*Math.pow(displayDensity,.3))*populationOpacity;let value;
   ...
   else value=Math.pow(displayDensity,.6);
