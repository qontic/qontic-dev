...
export function mountPacketEngine({core,advanced}){
 ...
 panel.innerHTML='<label>Source width σ <output>200 nm</output><input aria-label="Packet source width" type="range" min="50" max="400" step="5" value="200"></label><label>Packet length σ <output>50 nm</output><input aria-label="Packet length" type="range" min="5" max="200" step="5" value="50"></label><label>Slit width σ <output>30 nm</output><input aria-label="Packet slit width" type="range" min="10" max="200" step="5" value="30"></label><label><input type="checkbox" checked> Repeat packets</label><p class="packet-note">Gaussian source → apertures → screen. Each pulse contains independent particles. Some are absorbed.</p>';advanced.prepend(panel);
 const [sourceWidth,length,width,repeat]=panel.querySelectorAll('input');
 ...
 let enabled=true,p=null,coeff=null,profile=null,law=null,sourceGrid=null,fingerprint='',particles=[],pending=[],t=0,totalTime=0,pulse=0,absorbed=0,missed=0,last=null,realElapsed=0,timeUnit=1,yOffset=0,finished=false,packetAnimationId=null,visualPhase=0,flash=null,hold=0,lastMode=interpretation;
 ...
 function prepare(){
  t=0;visualPhase=0;finished=false;pulse++;particles=Array.from({length:p.particles},()=>{const a=sampleSource(p);a.schedule=quantumSchedule(a,p,law);return a;});nParticles+=p.particles;
 }
 ...
 function integrate(dt){
  ...
  for(const a of particles){
   ...
   if(interpretation==='bohmian'){
    ...
    a.done=true;if(a.schedule.type==='absorbed')absorbed++;else if(a.schedule.type==='missed')missed++;else pending.push(a.schedule.index);
   }
  }
 ...
 function frame(){
  ...
  const now=performance.now(),wallDt=Math.min(.25,last===null?0:(now-last)/1000);last=now;realElapsed+=wallDt;
 ...
 function ensembleOpacity(){return particles.length?Math.max(0,(particles.filter(a=>!a.done).length+pending.length)/particles.length):0;}
 ...
 function drawSourceMarker(){partCtx.save();partCtx.globalAlpha=1;partCtx.fillStyle='#ffe799';partCtx.strokeStyle='#263746';partCtx.lineWidth=2;partCtx.beginPath();partCtx.arc(X(0),Y(0),5,0,2*Math.PI);partCtx.fill();partCtx.stroke();partCtx.font='12px sans-serif';partCtx.strokeText('Source',X(0)+9,Y(0)-9);partCtx.fillText('Source',X(0)+9,Y(0)-9);partCtx.restore();}
 ...
 function draw(){if(!enabled)return;ensure();syncMode();setupCtx.clearRect(0,0,canvas.width,canvas.height);
  ...
  drawWave();drawParticles();drawSourceMarker();histogram();drawDetection();window.qonticScaleOverlay?.update();stats();
 }

 function hash(){return '&engine=packet&packetOrigin=source&sourceWidth='+sourceWidth.value+'&packetLength='+length.value+'&packetWidth='+width.value+'&packetCount='+p.particles;}
 const params=new URLSearchParams(location.hash.slice(1));
 for(const [key,el] of [['packetLength',length],['packetWidth',width],['sourceWidth',sourceWidth]]){const n=Number(params.get(key));if(n>=+el.min&&n<=+el.max){el.value=n;el.parentElement.querySelector('output').textContent=n+' nm';}}
 const count=+params.get('packetCount');if(count>=1&&count<=5000)$('#MaxPart-group')[0]?.setValueInFirstUnit(count);
 for(const control of [length,width,sourceWidth])control.addEventListener('input',()=>{control.parentElement.querySelector('output').textContent=control.value+' nm';resetEngine();draw();});
 queueMicrotask(()=>{
  ...
  const origin=document.getElementById('sourceOption');origin.value='isotropic';origin.querySelector('option[value=isotropic]').textContent='Gaussian source';
 ...
 return {get enabled(){return true;},reset:resetEngine,draw,drawWave,drawParticles,histogram,frame,updateMath,hash,pause(){last=null;if(packetAnimationId!==null)cancelAnimationFrame(packetAnimationId);packetAnimationId=null;}};
}
