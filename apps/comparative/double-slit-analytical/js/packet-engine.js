...
export function mountPacketEngine({core,advanced}){
 ...
 function emissionPeriod(){return +interval.value===0?Infinity:(+interval.value)*(particleType==='neutron'?1:.001)/timeUnit;}
 function prepare(){
 ...
 function resetEngine(){
  window.qonticMWBranches?.cancel();detectorFlashes.clear();pending=[];particles=[];pulses=[];nextEmission=0;visualPhase=0;flash=null;hold=0;lastMode=interpretation;p=config();fingerprint=JSON.stringify([p,screenHeight,particleType]);yOffset=screenHeight/200;
 ...
 function processPending(){
  ...
  if(interpretation==='manyworlds'&&window.qonticMWBranches?.enabled){
   ...
   const started=window.qonticMWBranches.begin({selected:index,weights:law.weights,detectorFraction:detectorX/canvas.width,sensorFraction:sensorWidth/canvas.width,
    ...
    onSelect(i){setRecord(i);detectorFlashes.clear();flash=null;last=null;draw();}
   });if(started)return;
  }
  addHit(index);detectorFlashes.set(index,.45);flash={index,remaining:.45};
 }
 ...
 function frame(){
  ...
  if(busy||(slow&&pending.length)){
   ...
   if(+interval.value===0&&particles.every(a=>a.done)&&!pending.length)prepare();
 ...
 function drawDetection(){
  ...
  // The sensor is on the setup layer, above the particle layer.
  // Retain every pixel registered this frame, not just the last one.
  setupCtx.save();setupCtx.fillStyle='#ffe476';
  for(const [index,remaining] of detectorFlashes){setupCtx.globalAlpha=Math.min(1,remaining/.15);setupCtx.fillRect(detectorX,index*canvas.height/p.bins,Math.max(1,sensorWidth),canvas.height/p.bins);}
  setupCtx.restore();
 }
