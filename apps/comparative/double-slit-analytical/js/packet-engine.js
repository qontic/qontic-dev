import {cachedPacketFrame,LaunchClock,emissionCone} from './packet-cache.js?v=45';
import {sourceSchedule,stepParticles} from './spherical-field.js?v=45';
...
export function mountPacketEngine({core,advanced}) {
 ...
 const focusLabel=document.createElement('label');focusLabel.hidden=true;focusLabel.innerHTML='<input type="checkbox" checked> Emit particles within slit cone (biased sample)';panel.insertBefore(focusLabel,note);
 ...
 let sphericalTable=null,sphericalFrame=null,sphericalWorker=null,sphericalError='',sphericalPreparing=false,sphericalProgress='',sphericalJob=0;
 ...
 function prepareSpherical(){
  ...
  sphericalPreparing=true;sphericalProgress='Checking saved spherical field…';sphericalWorker=new Worker(new URL('./spherical-worker.js?v=45',import.meta.url),{type:'module'});
  sphericalWorker.onmessage=({data:result})=>{
   if(job!==sphericalJob||!enabled||!p.spherical)return;
   if(result.progress){sphericalProgress=result.progress;stats();return;}
 ...
 function stats(){
  ...
  if(p.spherical&&(sphericalPreparing||sphericalError)){status.textContent=sphericalError||sphericalProgress||'Preparing spherical packet…';return;}
 ...
 function drawSourceMarker(){
  if(!p.source)return;
  partCtx.save();partCtx.globalAlpha=1;
  if(p.spherical&&p.focused){
   partCtx.strokeStyle='rgba(120,210,225,.5)';partCtx.lineWidth=1;partCtx.setLineDash([4,5]);
   for(const angle of emissionCone(p)){partCtx.beginPath();partCtx.moveTo(X(0),Y(0));partCtx.lineTo(X(p.wall),Y(p.wall*Math.tan(angle)));partCtx.stroke();}
   partCtx.setLineDash([]);
  }
  partCtx.fillStyle='#ffe799';partCtx.strokeStyle='#263746';partCtx.lineWidth=2;
  partCtx.beginPath();partCtx.arc(X(0),Y(0),5,0,2*Math.PI);partCtx.fill();partCtx.stroke();
  partCtx.font='12px sans-serif';partCtx.strokeText('Source',X(0)+9,Y(0)-9);partCtx.fillText('Source',X(0)+9,Y(0)-9);partCtx.restore();
 }
 ...
 function draw(){if(!enabled)return;ensure();setupCtx.clearRect(0,0,canvas.width,canvas.height);
  ...
  drawWave();drawParticles();drawSourceMarker();histogram();window.qonticScaleOverlay?.update();stats();
 }
 ...
 const params=new URLSearchParams(location.hash.slice(1));if(params.get('engine')==='packet'){
  ...
  const interval=Number(params.get('launchInterval'));if(interval>=.5&&interval<=10){launchInterval.value=interval;launchLabel.querySelector('output').textContent=interval.toFixed(1)+' s';}focus.checked=params.get('aimAtSlits')!=='0';
  // Defer activation until the adapter has been assigned to the main engine.
  queueMicrotask(()=>{select.value=params.get('packetOrigin')==='spherical'?'spherical':params.get('packetOrigin')==='source'?'source':'spherical';switchEngine();});
 }
