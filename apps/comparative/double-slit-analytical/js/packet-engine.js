...
import {aperture,sourceCoefficients,sourceComponents,sourceTransverse,sourceEnvelope,sampleSource,sampleTransmittedSource,stepSource,sourceProfile,maximumCoreSafeSeparation} from './source-packet-model.js?v=2.93-expert-balance';
import {detectorLaw,quantumSchedule,recordWithHit,samplePixel} from './packet-outcomes.js?v=2.93-expert-balance';
export function mountPacketEngine({core,advanced,expert}){
 ...
 function applyPacketLimits(){
  ...
  directedRow.title='PW postselection: shows only source configurations transmitted through the displayed ±'+n+'σ slit cores; it does not add a steering force.';
 ...
 const layoutStyle=document.createElement('style');layoutStyle.textContent='.packet-core-options{display:flex!important;align-items:center;gap:8px;margin:0 0 8px;padding:0 0 8px;border-bottom:1px solid var(--border-color,#476477)}.packet-core-options select{flex:1 1 86px;min-width:72px}.packet-core-options .packet-directed-inline,.packet-core-options .packet-slit-colors{display:flex!important;align-items:center;gap:4px!important;width:auto;min-width:0;margin:0!important;font-size:12px;white-space:nowrap}.packet-core-options .packet-directed-inline[hidden],.packet-core-options .packet-slit-colors[hidden]{display:none!important}.packet-core-options input{margin:0;flex:0 0 auto}#analytical-expert .packet-inline{display:grid;grid-template-columns:90px minmax(40px,1fr) minmax(58px,auto);gap:6px;align-items:center;margin:5px 0}#analytical-expert .packet-inline label{display:block;margin:0;font-size:12px}#analytical-expert .packet-inline input[type=range]{grid-column:auto;width:100%}#analytical-expert .packet-inline input[type=number]{width:58px}#analytical-expert .packet-inline output{font-size:12px}.packet-balance output{text-align:right;min-width:62px}';document.head.append(layoutStyle);
  ...
  if(wasPW!==isPW)for(let i=0;i<particles.length;i++){
   ...
   if(!isPW){a.schedule={at:(p.screen-a.x0)/p.k,type:'hit',index:samplePixel(law.weights)};a.path=[];}
    ...
    const b=crossedWall||directed.checked
     ?sampleTransmittedSource(p,Math.random,a.x0,directed.checked?p.slitExtentSigma:null)
 ...
 function drawParticles(){
  ...
  for(const a of particles){
   ...
   if(!opacity)a.path=[];
   if(tails&&a.path.length)buckets[colorBySlit.checked&&a.slitSide?a.slitSide:'default'][Math.min(8,Math.ceil(opacity*8))].push(a);
  }
  partCtx.save();partCtx.beginPath();partCtx.rect(0,0,detectorX,canvas.height);partCtx.clip();
  for(const group of ['default','upper','lower']){
   partCtx.strokeStyle=group==='default'?colorTraj:slitColors[group];
   for(let i=1;i<=8;i++){
  ...
  if(dots){partCtx.globalAlpha=elementOpacity('plot_particles');for(const group of ['default','upper','lower']){partCtx.fillStyle=group==='default'?colorPart:slitColors[group];partCtx.beginPath();for(const a of particles)if(!a.done&&(colorBySlit.checked&&a.slitSide?a.slitSide:'default')===group){const x=X(a.x),y=Y(a.y);partCtx.moveTo(x+3,y);partCtx.arc(x,y,3,0,2*Math.PI);}partCtx.fill();}}
  partCtx.restore();
 }
 ...
 for(const control of [length,width,sourceWidth,extent])control.addEventListener('input',()=>{applyPacketLimits();syncPacketInput(control);slitWidthHelp();resetEngine();draw();});
