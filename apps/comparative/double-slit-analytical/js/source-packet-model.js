...
function densityCdf(p,x,bound,coeff,samples=16385){
 const dy=2*bound/(samples-1),values=Array.from({length:samples},(_,i)=>sourceDensity(-bound+i*dy,x,p,coeff));
 const cdf=Array(samples).fill(0);
 for(let i=1;i<samples;i++)cdf[i]=cdf[i-1]+(values[i-1]+values[i])*dy/2;
 const total=Math.max(1e-300,cdf.at(-1));
 for(let i=0;i<samples;i++)cdf[i]/=total;
 return {bound,dy,cdf};
}
function cdfAt(table,y){
 const u=(y+table.bound)/table.dy;if(u<=0)return 0;if(u>=table.cdf.length-1)return 1;
 const i=Math.floor(u),f=u-i;return table.cdf[i]*(1-f)+table.cdf[i+1]*f;
}
function inverseCdf(table,q){
 q=Math.max(0,Math.min(1,q));let lo=0,hi=table.cdf.length-1;
 while(hi-lo>1){const mid=(lo+hi)>>1;if(table.cdf[mid]<q)lo=mid;else hi=mid;}
...
export function coreConditionedProfile(p,ymin,ymax,samples=2049,extentSigma=p.slitExtentSigma??1.5){
 ...
 // and detector. Transporting the core indicator through that exact quantile
 // map gives the selected ensemble's screen density without Monte Carlo noise.
 if(p.whichPath)return sourceProfile(p,ymin,ymax,samples);
 const coeff=sourceCoefficients(p),incidentSpread=sourceWidth(p.wall,p);
 const wallBound=Math.max(10*incidentSpread,...p.centers.map(c=>Math.abs(c)+10*p.sy));
 const travel=p.screen-p.wall,screenSpread=p.sy*Math.sqrt(1+(travel/(2*p.k*p.sy*p.sy))**2);
 const screenBound=Math.max(Math.abs(ymin),Math.abs(ymax),10*incidentSpread,...p.centers.map(c=>Math.abs(c)+12*screenSpread));
 const wall=densityCdf(p,p.wall,wallBound,coeff),screen=densityCdf(p,p.screen,screenBound,coeff),intervals=mergedSlitIntervals(p,extentSigma);
 const dy=(ymax-ymin)/(samples-1),values=Array.from({length:samples},(_,i)=>{
  const y=ymin+i*dy,yWall=inverseCdf(wall,cdfAt(screen,y));
  return intervals.some(([lo,hi])=>yWall>=lo&&yWall<=hi)?sourceDensity(y,p.screen,p,coeff):0;
 });
 const integral=values.reduce((sum,value,i)=>sum+value*((i===0||i===samples-1)?.5:1),0)*dy;
 return {values,integral};
}
