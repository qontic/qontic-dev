...
import {dragSurfaceGeometry} from './packet-interaction.js?v=2.99';
...
export function mountPacketSurface3D({host}){
 ...
 const overlay=document.createElement('div');overlay.className='packet-surface-help';overlay.innerHTML='<span>Drag: rotate · Wheel/pinch: zoom · Right-drag: pan</span><label>Height <select aria-label="3D surface height" title="Choose the wave quantity graphed as height. cos φ is continuous and avoids the artificial ±π wrap cliff. With which-path tagging, signed and phase heights are density-weighted component displays."><option value="psi2">|Ψ|²</option><option value="phase">cos φ</option><option value="real">Re Ψ</option><option value="imag">Im Ψ</option></select></label><button type="button" title="Reset 3D camera">Reset view</button>';
