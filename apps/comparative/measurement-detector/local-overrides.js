...
const updatedDetails = `
  ...
  <p><b>X is the system coordinate.</b> It labels the packet whose coarse position regions are being measured. <b>Y is the pointer coordinate.</b> The coupling shifts Y by an amount that depends on the X region, while the shifted packets remain overlapping. <b>Z is the record coordinate.</b> The readout correlates intervals of Y with separated Z channels.</p>
  <p>X, Y, and Z are coordinates of the compound system–apparatus configuration space; they are not three spatial directions of one particle. The X–Y view emphasizes the coupling, the X–Z view compares the system coordinate with the final record, and the Y–Z view shows how the pointer is recorded. The 3D view displays all three configuration-space coordinates together.</p>
  ...
  <p>B(y) partitions the Y axis into readout intervals. The impulsive unitary translates each interval into a distinct, well-separated Z packet at the readout plane. The Z position width σ<sub>z</sub> controls the resolution, and its allowed range keeps neighboring channels distinguishable. The readout acts directly on Y, not X; Z records the result of the complete weak measurement. Comparing Z with the earlier X region illustrates its statistical character. For visual clarity, the renderer does not brighten tails below 2.5% of a panel's peak; this display cutoff does not alter the analytical state.</p>
...
function installPermanentControls(root = document) {
  ...
  if (!panel) {
    ...
    panel.innerHTML = `
      ...
      <div class="xyz-view-control"><span>View</span><div><button class="active" data-view="xy" title="X–Y: system coordinate X and pointer coordinate Y; record coordinate Z is hidden" aria-label="X–Y view: system and pointer">X–Y</button><button data-view="xz" title="X–Z: system coordinate X and final record coordinate Z; pointer coordinate Y is hidden" aria-label="X–Z view: system and record">X–Z</button><button data-view="yz" title="Y–Z: pointer coordinate Y and record coordinate Z; system coordinate X is hidden" aria-label="Y–Z view: pointer and record">Y–Z</button><button data-view="3d" title="3D: full rotatable X–Y–Z configuration-space density (not ordinary physical 3-space)" aria-label="3D configuration-space view">3D</button></div><small class="view-axis-key"><b>X</b> system · <b>Y</b> pointer · <b>Z</b> record</small></div>`;
...
function installAdvancedDetectorControls(root = document) {
  ...
  wrap.innerHTML = `
    ...
    <label title="Pointer Y position width. Larger values increase overlap."><span>σy</span> <output>${localYPositionWidth.toFixed(2)}</output></label>
    <input aria-label="Pointer Y position width sigma y" data-control="sigmaY" type="range" min="0.35" max="1.5" step="0.02" value="${localYPositionWidth}">
    ...
    <input aria-label="Record Z position width sigma z" data-control="sigmaZ" type="range" min="0.12" max="0.45" step="0.01" value="${localZPointerWidth}">
    <label title="Duration of the X–Y coupling. Zero is the exact impulsive limit.">Duration τ <output>${localInteractionDuration === 0 ? 'Impulse' : localInteractionDuration.toFixed(2)}</output></label>
    ...
    <label title="X position of the Y–Z readout plane. A minimum gap is enforced.">Readout Pos. <output>${localStrongDetectorX.toFixed(2)}</output></label>
    ...
    <p class="detector-plane-note">Coupling and readout positions set the two interaction planes. They appear as planes in 3D and as labeled vertical lines in the X–Y and X–Z projections.</p>
