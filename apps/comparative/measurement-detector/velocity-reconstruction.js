...
function sampleRun() {
  ...
  const branch = Math.max(0, Math.min(vr.bins - 1,
    Math.floor((xf + 3 * vr.sigmaFinal) / (6 * vr.sigmaFinal) * vr.bins)));
  vr.runs.push({ xf, xWeak, estimate: (xf - xWeak) / vr.tau, branch });
...
function binned() {
  ...
  return Array.from({ length: vr.bins }, (_, i) => {
    ...
    const values = vr.runs.filter(r => r.xf >= lo + i * width && r.xf < lo + (i + 1) * width).map(r => r.estimate);
...
function install() {
  const permanent = document.querySelector('.permanent-controls-panel'), lab = document.querySelector('.lab');
  ...
  panel.innerHTML = `<header><div><b>Velocity reconstruction</b><small>Wiseman postselected ensemble</small></div><button data-vr-collapse aria-label="Collapse results">−</button></header>
    <div class="vr-body"><div class="vr-sequence"><span>Weak X</span><i>→ delay τ →</i><span>Final X (strong)</span></div><canvas id="vr-chart"></canvas><div class="vr-metrics"><span>Runs <b data-vr-count>0</b></span><span>RMSE <b data-vr-rmse>—</b></span><span data-vr-state>Paused</span></div>
